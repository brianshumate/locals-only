...
def test_empty_backend_version_still_distinguishes_machines(db):
    """The empty-version fallback matches on the other seven fields, so it
    never merges genuinely different hardware or backends."""
    db.upsert_environment(ENV)
    other_host = db.upsert_environment(
        {**ENV, "hostname": "yacht", "backend_version": ""})
    other_backend = db.upsert_environment(
        {**ENV, "backend": "lmstudio", "backend_version": ""})
    assert len({other_host, other_backend}) == 2
